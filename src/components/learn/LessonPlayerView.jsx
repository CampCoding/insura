...
export default function LessonPlayerView({ course, lesson }) {
  ...
  useLayoutEffect(() => {
    ...
    mm.add(
      ...
      (context) => {
        ...
        if (isDesktop) {
          gsap.set(el, { xPercent: 0, width: open ? 320 : 0 });
        } else {
          gsap.set(el, { width: 320, xPercent: open ? 0 : -100 });
        }

        animateRef.current = (nextOpen) => {
          if (isDesktop) {
            gsap.to(el, {
              width: nextOpen ? 320 : 0,
              duration: 0.45,
              ease: "power3.out",
            });
          } else {
            gsap.to(el, {
              xPercent: nextOpen ? 0 : -100,
              duration: 0.4,
              ease: "power3.out",
            });
          }
  ...
  useEffect(() => {
    animateRef.current?.(sidebarOpen);
  }, [sidebarOpen]);
  ...
  return (
    <div className="flex h-dvh flex-col overflow-hidden bg-background">
      <header className="flex h-16 shrink-0 items-center justify-between gap-4 border-b border-border bg-surface px-4">
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full text-foreground transition-colors hover:bg-primary-tint hover:text-primary"
          >
            {sidebarOpen ? (
          ...
          <Logo imageClassName="h-7" className="ml-1 hidden sm:flex" />

          {!sidebarOpen && (
            <div className="group relative ml-1 flex min-w-0 cursor-default items-center gap-1.5 rounded-full bg-primary-tint px-3 py-1.5 text-primary">
              <MonitorPlay size={14} className="shrink-0" strokeWidth={1.75} />
              <span className="max-w-[100px] truncate text-xs font-medium sm:max-w-[220px] sm:text-sm">
                {lesson.title}
              </span>
      ...
      <div className="relative flex flex-1 overflow-hidden">
        ...
        <aside
          ref={sidebarRef}
          className="fixed inset-y-0 top-16 left-0 z-30 shrink-0 overflow-hidden border-r border-border bg-surface lg:static lg:top-0"
        >
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-7xl px-6 py-8">
            ...
            <div
              ...
              className="relative select-none overflow-hidden rounded-card bg-foreground"
            >
              <video
                key={lesson.key}
                controls
